...
Shader "Crest/Ocean URP"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			// NOTE: Screen-Space Shadows no longer exist for URP since 7.2.
			...
			Varyings Vert(Attributes input)
			{
				...
				o.positionWS_fogFactor.y += seaLevelOffset;
			...
			half4 Frag(const Varyings input, const bool i_isFrontFace : SV_IsFrontFace) : SV_Target
			{
...
#if _CLIPSURFACE_ON
				{
					...
					PosToSliceIndices(input.positionWS_fogFactor.xz, 0, _SliceCount - 1.0, _CrestCascadeData[0]._scale, slice0, slice1, alpha);
					...
					if (weight0 > 0.001)
					{
						const float3 uv = WorldToUV(input.positionWS_fogFactor.xz, cascadeData0, slice0);
					...
					if (weight1 > 0.001)
					{
						const float3 uv = WorldToUV(input.positionWS_fogFactor.xz, cascadeData1, slice1);
				...
				for (int i = 0; i < pixelLightCount; ++i)
				{
					...
					Light light = GetAdditionalLight(i, input.positionWS_fogFactor.xyz, unity_ProbesOcclusion);
